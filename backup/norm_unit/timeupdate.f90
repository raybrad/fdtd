...
!	//////////////////////////// TF_SF source amplitude //////////////////////////
	if(TF_SF_tdecay == 0 .and.  t>=TF_SF_to) then !// CW source
	  Hy_inc(ks) =amplitude*sqrt(TF_SF_eps_bg*eo/uo)*sin( 2*pi*TF_SF_freq*(t-TF_SF_to)/(S_factor*ds_x*lattice_x) )		
	else if(TF_SF_to-3*TF_SF_tdecay<t .and. t<TF_SF_to+3*TF_SF_tdecay) then !// Gaussian pulse
	  if (.not. triple) then
	  	Hy_inc(ks) =amplitude*sqrt(TF_SF_eps_bg*eo/uo)*sin( 2*pi*TF_SF_freq*(t-TF_SF_to)/(S_factor*ds_x*lattice_x) )*exp(-1.0*(dble(t-TF_SF_to)/dble(TF_SF_tdecay))**2.0)
	  else
	     Hy_inc(ks)=0.d0
	     do nn=1,3
	     Hy_inc(ks)=Hy_inc(ks)+amplitude*sqrt(TF_SF_eps_bg*eo/uo)*sin( 2*pi*TF_SF_freq*(t-TF_SF_to)/(S_factor*ds_x*lattice_x)*dble(nn*0.5))*exp(-1.0*(dble(t-TF_SF_to)/dble(TF_SF_tdecay))**2.0)
	     enddo
	 endif
	else 
		Hy_inc(ks) = Hy_inc(ks)
...
real*8:: temp_0, temp_1, temp_p1, temp_p0
	...
	if(TF_SF_tdecay == 0 .and. t>=TF_SF_to) then !// CW source      !pi is the source of -Ex
		Ex_inc(ks) =amplitude*sin(pi+2*pi*TF_SF_freq*(t-TF_SF_to)/(S_factor*ds_x*lattice_x) )		
	else if(TF_SF_to-3*TF_SF_tdecay<t .and. t<TF_SF_to+3*TF_SF_tdecay) then ! // Gaussian pulse
	       if (.not. triple) then
		Ex_inc(ks) =amplitude*sin(pi+2*pi*TF_SF_freq*(t-TF_SF_to)/(S_factor*ds_x*lattice_x) )*exp(-1.0*(dble(t-TF_SF_to)/dble(TF_SF_tdecay))**2.0)
	       else
		 Ex_inc(ks)=0.d0
		 do nn=1,3
		 Ex_inc(ks)=Ex_inc(ks)+amplitude*sin(pi+2*pi*TF_SF_freq*(t-TF_SF_to)/(S_factor*ds_x*lattice_x)*dble(nn*0.5))*exp(-1.0*(dble(t-TF_SF_to)/dble(TF_SF_tdecay))**2.0)
	         enddo
	       endif
...
real*8:: sigma_maxl, sigma_maxr
	 ...
	 ! // no use of periodic boundary condition 
		...
		if(a<=pmljl) then
		      sigmay=sigma_maxl*(-(a-pmljl)/pmljl)**orderxl
		elseif(a<=jsize-pmljr) then 
		      sigmay=0.0
		else
		      sigmay=sigma_maxr*((a-(jsize-pmljr+1))/pmljr)**orderxr
                endif
...
real*8:: sigma_maxl, sigma_maxr
	...
	sigma_maxr=(sig_azr)*(orderzr+1)/(120*pi*sqrt(back_epsilon))
		...
		if(a<=pmlkl) then
		      sigmaz=sigma_maxl*(-(a-pmlkl)/pmlkl)**orderxl
		elseif(a<=ksize-pmlkr) then 
		      sigmaz=0.0
		else
		      sigmaz=sigma_maxr*((a-(ksize-pmlkr+1))/pmlkr)**orderxr
                endif
end function sigmaz
...
real*8::kappa_maxl,kappa_maxr
	...
	kappa_maxr=14
	if(a<=pmlil) then
		kappax=1.0+(kappa_maxl-1)*(-(a-pmlil)/pmlil)**orderxl
	elseif(a<=isize-pmlir) then 
		kappax=1.0
	else
...
real*8::kappa_maxl,kappa_maxr
	...
	kappa_maxr=14
	if(a<=pmljl) then
		kappay=1.0+(kappa_maxl-1)*(-(a-pmljl)/pmljl)**orderyl
	elseif(a<=jsize-pmljr) then 
		kappay=1.0
	else
...
real*8::kappa_maxl,kappa_maxr
	...
	kappa_maxr=14
	if(a<=pmlkl) then
		kappaz=1.0+(kappa_maxl-1)*(-(a-pmlkl)/pmlkl)**orderzl
	elseif(a<=ksize-pmlkr) then 
		kappaz=1.0
	else
